...
type BoardColumnRepository interface {
	...
	Create(issuerId uint, data *model.BoardColumn) error
	Update(id uint, issuerId uint, data *dto.BoardColumnRequest) error
...
func (b *boardColumnRepository) Create(issuerId uint, data *model.BoardColumn) error {
	var board model.Board
	// get board members
	if err := b.db.Preload("Members").Where("id = ?", data.BoardID).First(&board).Error; err != nil {
		return err
	}
...
func (b *boardColumnRepository) Update(id uint, issuerId uint, data *dto.BoardColumnRequest) error {
	var column model.BoardColumn
	var board model.Board
